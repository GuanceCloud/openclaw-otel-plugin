...
test("stringAttrs maps openclaw fields to canonical aliases", () => {
  const attrs = stringAttrs({
    ...
    "openclaw.skill.description": "生成监控器",
    "openclaw.skill.path": "/tmp/workspace/skills/monitor/SKILL.md",
...
test("traceAttrs keeps canonical context fields while dropping redundant legacy context keys", () => {
  const attrs = traceAttrs({
    ...
    "skill.description": "生成 Dashboard",
  ...
  assert.equal(attrs["skill.name"], "dashboard");
  assert.equal(attrs["skill.description"], "生成 Dashboard");
  assert.equal(attrs["skill.path"], "/tmp/workspace/skills/dashboard/SKILL.md");
  assert.equal(attrs["skill.source.type"], "workspace");
