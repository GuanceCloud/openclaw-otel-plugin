...
- `headers` 可用于给 trace 和 metrics 上报统一附加 HTTP Header
- `agentProvider` 默认为 `openclaw`，会作为全局资源标签 `agent_provider` 附加到 trace 和 metrics
- `globalTags` 用于追加固定全局标签，适合配置团队、集群、环境标识等
